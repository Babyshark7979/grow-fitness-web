...
const sheetId = process.env.GROW_REPORTING_SHEET_ID;
const title = process.env.INTERNAL_DASHBOARD_TITLE || 'Điều Hành Grow Fitness';

if (!inputPath || !outputPath) {
  throw new Error('Usage: node scripts/encrypt-internal-dashboard.mjs <input.html> <output.html>');
}
if (!password) {
  throw new Error('INTERNAL_DASHBOARD_PASSWORD is required.');
}
if (!sheetId) {
  throw new Error('GROW_REPORTING_SHEET_ID is required.');
}

const source = fs.readFileSync(inputPath, 'utf8');
if (!source.includes('__SHEET_ID__')) {
  throw new Error('Template is missing the __SHEET_ID__ placeholder.');
}
const plaintext = Buffer.from(source.replaceAll('__SHEET_ID__', sheetId), 'utf8');
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    input { width:100%; border:1px solid var(--line); border-radius:13px; outline:0; padding:14px 15px; background:#0c080b; color:var(--text); font-size:18px; letter-spacing:.13em; }
...
<body>
  <div id="gate">
    <main class="box">
      ...
      <p class="eyebrow">Bảng điều hành nội bộ đã mã hóa</p>
      <h1>Mở Điều Hành Grow</h1>
      <p>Nhập mật khẩu để xem biến động, báo cáo và việc cần làm theo tuần, tháng, quý.</p>
      <label for="pw">Mật khẩu truy cập</label>
      <input id="pw" type="password" placeholder="••••••" autocomplete="current-password" autofocus />
      <button id="btn" type="button">Giải mã và mở bảng</button>
      <div class="err" id="err" role="alert"></div>
      <div class="security"><span>🔐</span><span>AES-256-GCM · Mật khẩu chỉ được xử lý trên thiết bị này.</span></div>
    </main>
  </div>
  <iframe id="frame" title="${title}"></iframe>
  <script>
    const BLOB="${blob}";
    async function unlock() {
      ...
      try {
        ...
        button.textContent='Giải mã và mở bảng';
      }
    }
    document.getElementById('btn').addEventListener('click',unlock);
    document.getElementById('pw').addEventListener('keydown',event=>{if(event.key==='Enter')unlock();});
  </script>
</body>
</html>`;

fs.writeFileSync(outputPath, wrapper);
console.log(`[INTERNAL DASHBOARD] Wrote encrypted dashboard to ${outputPath}`);
